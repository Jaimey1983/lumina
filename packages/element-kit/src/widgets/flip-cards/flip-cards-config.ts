...
import { stableWidgetChildId } from '@lumina/editor-shared/widget-identity';
...
export function createDefaultFlipCard(index?: number, id?: string): FlipCard {
  const i = index ?? 1;
  return {
...
const DEFAULT_FLIP_CARDS_COUNT = 3;

export function padFlipCardsTarjetas(block: FlipCardsWidget): FlipCard[] {
  const current = block.tarjetas ?? [];
  if (current.length > 0) return current;
  return Array.from({ length: DEFAULT_FLIP_CARDS_COUNT }, (_, i) =>
    createDefaultFlipCard(i + 1, stableWidgetChildId(block, 'tarjeta', i + 1)),
  );
}
...
export function normalizeFlipCardsWidget(block: FlipCardsWidget): FlipCardsWidget {
  ...
  const withConfig: FlipCardsWidget = {
    ...block,
    configuracion: {
  ...
  return { ...withConfig, tarjetas: padFlipCardsTarjetas(withConfig) };
}
