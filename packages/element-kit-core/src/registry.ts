...
export class ElementRegistry<
  TCatalog extends Record<string, unknown> = Record<string, unknown>,
> {
  ...
  registrar<TKey extends keyof TCatalog & string, TState, TConfig>(
    definicion: ElementDefinition<TState, TConfig> &
      TCatalog[TKey] & { readonly tipo: TKey },
  ): void {
  ...
  obtener<TKey extends keyof TCatalog & string>(
    tipo: TKey,
  ): TCatalog[TKey] | undefined {
