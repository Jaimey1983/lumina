...
- **Estado:** en revisión — 11 `ElementDefinition` (clasificar, memoria, puzzle_imagen, sopa_letras, crucigrama, abrir_caja, ahorcado, puzzle_palabras, globos, topo, historia_ramificada) con `puntuacion` → `evaluateActivityResponse`. Paridad por actividad. `TODO(migración-etapa-5)` por fila (ticket `LUM-E5-GRUPO4`, 2026-11-30). Verif: `pnpm --filter @lumina/element-kit test` → **69/69** · lint 0 · `pnpm --filter lumina-frontend build` OK.
...
- **Estado:** [en curso: Claude Code] — **PAUSADA**: colisión con E2.4 en el plumbing compartido de `packages/element-kit` (`src/index.ts`, `tsconfig*`, `vitest.config.ts`) + `lumina-frontend/package.json`. E2.4 y E2.5 no son disjuntas ahí (Regla 10). Se retoma cuando E2.4 esté `hecho`. Tranche 1 (frontend: `activity-templates.ts` + `element-kit-classic.ts` + barrel en `package.json`) ya está en disco sin commitear, se rebasa encima de E2.4.
