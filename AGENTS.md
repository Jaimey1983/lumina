...
- **Estado:** [en curso: Claude Code] — **reanudada** (E2.4 `hecho`, `30ed4e5`). Tranche 1 (frontend: `activity-templates.ts` + `element-kit-classic.ts` + barrel `./editor-activities` en `package.json` + `editor-client.tsx` importa las plantillas) está en disco sin commitear, se rebasa sobre E2.4. Falta tranche 2: las 10 `ElementDefinition` clásicas en `packages/element-kit/src/elements/<tipo>/` + registro + parity spec, encima del `src/index.ts` que ya trae las 12 de E2.3/E2.4.
