...
- **Operador:** Cursor
- **Estado:** en revisión — migrado clip-group a ElementDefinition sin Paper.js (barrel lumina-frontend/src/components/clip-group/index.ts, subpath ./blocks/clip-group, shim + alias ×3, adapters + definition + register en element-kit, registro único en ElementRegistry, TODO en slide-renderer.tsx junto a case 'clip-group', paridad de DOM en clip-group.parity.spec.tsx). Verificación en aislamiento: element-kit build/lint OK, test 202/202; frontend lint 0 errores/70 warnings, test:unit 446/446, build OK.
