...
#### E5 — Unificar estado del editor · **RAÍZ REDACTADA** · sub-fichas E5.1–E5.7 redactadas (Cursor) y **revisadas por Claude Code**. **E5.1 hecho** (`3713276`) · **E5.2 hecho** (`7a252c2`) · **E5.3 hecho** (`ca56829`) · **E5.4 hecho** (`293d5f1`) · **E5.5 devuelto a Cursor** (revisión Claude Code: 2 regresiones + gate `element-kit test` rojo + 8 deudas listadas en la ficha; 32 archivos sin commitear en el árbol) · E5.6 tras E5.5 · E5.7 lista · E5.6 con correcciones incorporadas (opción 2 para el hueco de metadata: registros viejos se adelgazan en E5.5, borrado completo → E7). Fuera de E5: **X.1 hecho** (`canvas-editor.tsx` borrado).
...
- **Operador:** Cursor
- **Estado:** **devuelto a Cursor por revisión de Claude Code (2026-09-06)** — NO aprobado. El código se commiteó concurrente durante la revisión (`d570527`, ya empujado). **Re-verificación en aislamiento (Claude Code):** todos los gates VERDES — frontend `tsc` limpio · `pnpm lint` **0 errores** (62 warnings) · `test:unit` **473/473** · `pnpm build` OK 17/17 · `pnpm --filter @lumina/element-kit test` **266/266** (los "3 failed" del primer intento eran **contención** por correr en paralelo con `tsc`/`lint`, no una regresión real — **deuda 8 descartada**). **Pero los gates verdes NO limpian las regresiones** (debajo): ninguna prueba cubre el wiring canvas↔panel. Deudas que persisten antes de re-enviar:
  1. **Regresión `onEnsureBlockSelected` → no-op** en 7 widgets (flip-cards, tabs, carousel, click-reveal, popup, hotspot, timeline): los adapters del kit hardcodean `onEnsureBlockSelected={() => undefined}`. Click dentro del widget ya no selecciona el bloque en el editor.
  2. **Regresión inner-selection**: `SlideRenderer` sigue declarando/destructurando `flipCardsInnerSelection`/`on*InnerSelectionChange` (líneas ~1278–1338) pero el dispatch genérico ya no los reenvía; `editorState.inner.*` (reducer, E5.2) queda huérfano. La sub-edición inline dentro del widget sigue (adapter local); el wiring canvas↔panel se corta. **Decisión requerida (Regla 10):** wire inner-selection por `config` del contrato, o diferir la sub-edición lateral con `TODO(migración-etapa-6/7)` + ticket — no dejarlo roto en silencio.
  3. **Type-safety**: `RenderActivity` usa `obtener<Activity, Record<string, unknown>>` — el cast anula el chequeo de tsc sobre la forma del `config` de las 22 actividades. Mismatch → runtime. Endurecer o cubrir con smoke.
  4. `handleWidgetChange` sin case para `boton`/`contador`/`progreso`/`tooltip`/`ruleta` → `default: break` descarta su `onChange` (verificar que esos editan solo por panel).
  5. **Imports muertos** nuevos en `slide-renderer.tsx` (`Background`, `TooltipWidget`, `BLOCK_FALLBACKS`, `POPUP_TRIGGER_PX_MAX/MIN`, `_`) — limpiar.
  6. Los paneles que la ficha lista como consumidores de dispatch (`flyout-panel.tsx`, `panels/activities-panel.tsx`, `panels/widgets-insert-panel.tsx`) **no se tocaron** — confirmar si hacía falta o si la ficha los sobre-listó.
  7. BOM (U+FEFF) al inicio de `element-registry-bootstrap.ts` — quitar.
  8. ~~Verificación completa verde~~ **RESUELTO** — todos los gates verdes en aislamiento (ver Estado). Queda: mantenerlos verdes tras corregir 1–7.
  Lo que quedó bien y no hay que rehacer: `element-registry-bootstrap.ts` (patrón side-effect + `-core`, cycle-free), `next.config.ts` + deps de `package.json`, los 3 `case` canvas intactos con su `RIESGO ACEPTADO`, los 24 repoints `@/lib/activity-scoring` → `@lumina/scoring` + borrado de la fachada.
