...
- **Operador:** Antigravity
- **Estado:** **en revisión** — 8 primitivos migrados a `@lumina/element-kit` (`texto`, `imagen`, `video`, `audio`, `codigo`, `cita`, `separador`, `columnas`) con sus barrels/adapters/shims/parity specs (8 suites nuevas, 38/38 suites y 314 tests verdes en element-kit). Segundo `switch` por `act.tipo` (400 líneas de código muerto) en `slide-renderer.tsx` eliminado (`LUM-E5-CLASICAS` cerrado), conservando `torneo` y `escape_room`. Primitivos despachados vía `elementRegistry` en `slide-renderer.tsx` y `properties-panel.tsx`. `TODO(migración-etapa-5)` en `activity-templates.ts` limpiado. Verificación corrida: `pnpm --filter @lumina/element-kit build && pnpm --filter @lumina/element-kit test && pnpm --filter @lumina/element-kit lint && cd lumina-frontend && npx tsc --noEmit && pnpm lint && pnpm test:unit && pnpm build` (todos en código 0, lint 0 errores, unit tests 495/495, build 17/17 páginas).
