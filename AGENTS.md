...
**Tensiones / decisiones a resolver ANTES de asignar sub-fichas (Regla 10 — no las decide el operador):**

1. **Inversión de la dependencia `element-kit ↔ lumina-frontend`.** Opciones: **(a)** mover al kit los componentes/tipos que hoy consume por shim y dejar al frontend importando solo `@lumina/element-kit`; **(b)** partir el kit en `@lumina/element-kit-core` (contrato + `ElementRegistry`, sin React ni frontend) que el frontend importa, dejando los adapters concretos donde puedan seguir dependiendo del frontend hasta E7. **Recomendación de la ficha raíz: (b)** — desacopla el registry del canvas sin migración masiva en E5.
2. **Alcance del "reducer central": ¿un slide o el mazo?** **Recomendación:** gobierna **el slide en edición** (bloques + fondo + guías + selección + historial); la lista de slides sigue en react-query; la "diferencia" es a nivel de bloque dentro del slide. El reducer del mazo entero es E6+ (Core↔Edu).
3. **"Persistencia por diferencia" y el contrato backend.** **(a)** E5 solo empieza a **mandar `expectedVersion`** y serializa un `content` mínimo desde el reducer, sin tocar el DTO; **(b)** endpoint `ops:[{add|update|remove, blockId,…}]` por bloque. **Recomendación: (a)** en E5 (frontend calcula el diff, backend sigue recibiendo `content` + versión); (b) se evalúa en E6 — toca `lumina-backend` y saca a E5 de "frontend puro".
4. **Regla 1 vs. deadlock (misma lectura que E4).** "Cerrada" para una sub-ficha de E5 = el `case` / fila / fachada **efectivamente borrado** y su prueba de paridad reapuntada al `elementRegistry`. Lo que no se pueda borrar por el `RIESGO ACEPTADO (E4.5 §2)` **bloquea su propia sub-ficha** hasta que exista la cobertura de integración — no se hereda otro `TODO`.
5. **Primitivos (E4.6) vs. descongelar el switch.** **Recomendación:** extraerlos dentro de E5, en la misma pasada que borra el `switch` (freeze ya levantado). Son primitivos sin scoring, patrón E3.
...
- **E5.1** — Desacoplar el registry (opción de Tensión 1). Frontend importa `elementRegistry` sin ciclo. Sin cambio de comportamiento; `build` de los 3 paquetes verde.
