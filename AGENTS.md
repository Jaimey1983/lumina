...
- **Operador:** Cursor
- **Estado:** **en revisión** — Cursor corrigió deudas 1–7 encima de `d570527`. **Deuda 2 = (a)** wire por `config`: adapters leen `innerSelection`/`onInnerSelectionChange` (si no vienen, `useState` local para parity); `SlideRenderer` reenvía las props que ya recibía. Deuda 1: `onEnsureBlockSelected` por `config` (ya no no-op). Deuda 3: `ActivityRuntimeConfig` + smoke 22 actividades. Deuda 4: boton/contador/progreso/tooltip/ruleta editan solo por `properties-panel.applyNow` (sus Editors no llaman `onChange`; `default:break` correcto). Deuda 5: imports muertos borrados. Deuda 6: la ficha sobre-listó los paneles — son catálogo/inserción (metadata), no instancian Editor/Viewer. Deuda 7: BOM quitado. Verif aislada (secuencial): `cd lumina-frontend && npx tsc --noEmit` OK · `pnpm lint` **0 error** (56 warnings, sin los 6 de imports muertos) · `test:unit` **495/495** (473 + 22 smoke) · `pnpm build` OK 17/17 · `pnpm --filter @lumina/element-kit test` **266/266**.
