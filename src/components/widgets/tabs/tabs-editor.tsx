...
export function TabsEditor({
  ...
  const activeIndex = Math.min(configuracion.fichaActiva, Math.max(0, fichas.length - 1));
  ...
  const setActiveIndex = (index: number) => {
    patchWidget((w) => ({
      ...w,
      configuracion: { ...w.configuracion, fichaActiva: index },
    }));
  };
          ...
          {fichas.map((ficha, index) => (
            <button
              ...
              onClick={(e) => {
                ...
                onInnerSelectionChange?.({ kind: 'slide', slideId: ficha.id });
              }}
            >
              {ficha.etiqueta}
            </button>
