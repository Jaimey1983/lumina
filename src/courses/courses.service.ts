  ...
  export class CoursesService {
    ...
    async create(dto: CreateCourseDto, teacherId: string) {
      ...
      return this.prisma.course.create({
        data: { name: dto.name, description: dto.description, code: dto.code, teacherId },
        select: { id: true, name: true, description: true, code: true, isActive: true, teacherId: true, createdAt: true },
      });
    }
    ...
    async findAll(page = 1, limit = 20, userId: string, role: string) {
      ...
      const [courses, total] = await Promise.all([
        this.prisma.course.findMany({
          ...
          select: {
            id: true, name: true, description: true, code: true, isActive: true, createdAt: true,
    ...
    async findOne(id: string) {
      const course = await this.prisma.course.findUnique({
        where: { id },
        select: {
          id: true, name: true, description: true, code: true, isActive: true, createdAt: true, updatedAt: true,
          teacher: { select: { id: true, name: true, lastName: true, email: true } },
          _count: { select: { enrollments: true, classes: true } },
        },
      });
      if (!course) throw new NotFoundException(`Curso con ID ${id} no encontrado`);
      return course;
    }
  
    async update(id: string, dto: UpdateCourseDto, userId: string, role: string) {
      const course = await this.findOne(id);
      if (role !== 'ADMIN' && role !== 'SUPERADMIN' && course.teacher.id !== userId) {
      ...
      return this.prisma.course.update({
        ...
        select: { id: true, name: true, description: true, code: true, isActive: true, updatedAt: true },
      });
    }
    ...
    async enrollStudent(courseId: string, dto: EnrollStudentDto) {
      ...
      if (existing) throw new ConflictException('El estudiante ya está matriculado en este curso');
