...
import type { ApiGradebookRow } from '@/hooks/use-gradebook';
...
// ─── Scale conversion (same as use-gradebook.ts) ─────────────────────────────

function toDisplayNoteOnFive(score: number, maxScore: number): number | null {
  if (!Number.isFinite(score)) return null;
  const s = Number(score);
  const m = Number(maxScore);
  if (!Number.isFinite(m) || m <= 0) return Math.min(5, Math.max(0, s));
  if (m <= 5.0001) return Math.min(5, Math.max(0, s));
