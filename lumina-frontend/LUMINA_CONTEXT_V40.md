# LUMINA_CONTEXT_V40.md
> Generado: 22/08/2026
> Reemplaza: LUMINA_CONTEXT_V39.md
> Sprint incorporado: 17–22 agosto 2026
> Roadmap oficial del proyecto: `LUMINA_ROADMAP_CORE_VS_2.0.md` (generado 22/08/2026) —
> Parte A (CORE, 14 fases, prioridad de ejecución) + Parte B (visión Lumina 2.0). Reemplaza
> cualquier checklist o roadmap anterior. Este documento resume el estado; el detalle de tareas
> vive en ese archivo.
...
3. **Siempre leer contexto primero** — todo prompt debe iniciar con `"Lee LUMINA_CONTEXT_V40.md antes de empezar."`. Si la tarea viene del roadmap, agregar también `"Lee LUMINA_ROADMAP_CORE_VS_2.0.md, Fase X."`.
...
> **El roadmap oficial y detallado del proyecto es `LUMINA_ROADMAP_CORE_VS_2.0.md`**
> (generado 22/08/2026), dividido en:
> - **Parte A — CORE** (14 fases, orden de ejecución oficial: BYOK → Clipping Masks → Auditoría
>   de canvas → Grupo 8 polish → Escape Room 2.0 → Scoring/Edu → Sesiones/gamificación de sala →
>   Diagramas pedagógicos → Contenido enriquecido nativo → Experiencia docente → Experiencia
>   estudiante → Accesibilidad → Infraestructura → Mecánicas de juego a verificar).
> - **Parte B — LUMINA 2.0** (visión de expansión: IA más allá de BYOK, Community, gamificación
>   global cross-clase, contenido embebido de terceros, viewer inmersivo/fondos).
>
> Reemplaza cualquier checklist o roadmap anterior (incluido el listado por grupos que tenían
> el V38/V39). Esta sección solo resume el estado actual y el siguiente paso — para el detalle
> completo de tareas de cada fase, leer ese archivo.
...
### 🔜 SIGUIENTE (Fase 1 del roadmap oficial, Parte A)
...
`LUMINA_ROADMAP_CORE_VS_2.0.md` → Fase 1 para las 10 tareas completas (aún en diseño: qué pasa
...
- Registrada en `LUMINA_ROADMAP_CORE_VS_2.0.md` → Backlog → Scoring y Lumina Edu.
...
```
[FRONTEND] Lee LUMINA_CONTEXT_V40.md y CLAUDE.md antes de empezar.
[descripción del cambio]
Build debe pasar sin errores TypeScript. Verificar con npx tsc --noEmit.

[BACKEND] Lee LUMINA_CONTEXT_V40.md y .cursorrules antes de empezar.
[descripción del cambio]
Build debe pasar sin errores TypeScript. Verificar con npx tsc --noEmit.
...
Si el prompt corresponde a una fase del roadmap oficial, agregar además:
```
Lee LUMINA_ROADMAP_CORE_VS_2.0.md, [Parte A/B] → Fase N, antes de empezar.
```
