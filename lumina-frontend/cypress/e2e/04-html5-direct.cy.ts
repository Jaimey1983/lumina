...
describe('Video Interactivo: HTML5 Direct Video', () => {
  ...
  describe('HTML5 Adapter Initialization', () => {
    it('should detect and initialize direct HTML5 video provider', () => {
      cy.window().then((win) => {
        asTestWindow(win).__testVideoUrl = HTML5_PROVIDERS[0].url;
      });
    ...
    it('should autoplay after user starts activity (Capa 1)', () => {
      ...
      cy.window().then((win) => {
        const logs = asTestWindow(win).__videoEventLogs || [];
        const playEvents = logs.filter((l) => l.event === 'playback-resume');
  ...
  describe('HTML5 Seek Behavior', () => {
    it('should handle seek events on HTML5 element', () => {
      ...
      cy.window().then((win) => {
        const video = asTestWindow(win).currentHtml5Player as HTMLVideoElement;
        if (video) {
      ...
      cy.window().then((win) => {
        const logs = asTestWindow(win).__videoEventLogs || [];
        const seekEvents = logs.filter((l) => l.event === 'time-update');
  ...
  describe('HTML5 Playback Controls', () => {
    ...
    it('should handle video errors gracefully', () => {
      cy.window().then((win) => {
        asTestWindow(win).__testVideoUrl = 'https://example.com/nonexistent-video.mp4';
      });
  ...
  describe('HTML5 Cleanup on Unmount', () => {
    it('should properly cleanup video element on route change', () => {
      ...
      cy.window().then((win) => {
        const video = asTestWindow(win).currentHtml5Player;
