/**
 * Contrato del evaluador único (Fase 0).
 *
 * TODO(workspace): extraer este módulo + fixtures a un paquete compartido
 * (`packages/activity-scoring`) cuando el monorepo lo permita, para no
 * re-portar a mano. Hasta entonces, el espejo backend vive en
 * `lumina-backend/src/classes/activity-scoring.ts` y los fixtures se
 * sincronizan con `scripts/check-fixtures-sync.mjs`.
 */
