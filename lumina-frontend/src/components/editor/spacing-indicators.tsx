...
import {
  ...
  overlapsVertically,
  type RectPx,
} from '@/lib/canvas-spacing';

interface SpacingIndicatorsProps {
  activeBlock: Block;
  allBlocks: Block[];
  /** Índice en `allBlocks` — evita comparar por referencia de objeto. */
  activeIndex?: number;
...
export function SpacingIndicators({
  ...
  const indicators = useMemo(() => {
    ...
    const otherBlocks = allBlocks.filter((b, i) =>
      activeIndex !== undefined ? i !== activeIndex : b !== activeBlock,
    );
    ...
    // ─── 3. Find closest neighbors in 4 directions ────────────────────────────
        ...
        for (let i = 0; i <= hAligned.length - len; i++) {
          ...
          if (activeIdxH >= i && activeIdxH <= j) {
            ...
            if (valid) {
              ...
              if (maxGap - minGap <= SPACING_EQUAL_TOLERANCE_PX) {
                hEqualSpacingGroup = [];
                for (let k = i; k <= j; k++) {
    ...
    return lines;
  }, [activeBlock, activeIndex, allBlocks, canvasWidth, canvasHeight]);
