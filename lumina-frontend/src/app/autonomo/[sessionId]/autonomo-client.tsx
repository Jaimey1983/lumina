...
function ViewerScreen({
  ...
  const navigateSlide = useCallback(
    (action: SlideNavAction) => {
      ...
      if (action.kind === 'anterior') {
        if (!allowBackNav) return;
        setIdx((i) => Math.max(0, i - 1));
        setLocked(false);
        return;
      }
      if (!allowBackNav && action.index < idx) return;
      setIdx(Math.min(slides.length - 1, Math.max(0, action.index)));
      setLocked(false);
    },
  ...
  const handleResponse = useCallback((response: unknown) => {
    ...
    if (activityType === 'video_interactivo') {
      ...
      const current =
        response && typeof response === 'object' && !Array.isArray(response)
          ...
          : {};
      const qIdx = typeof current.questionIndex === 'number' ? current.questionIndex : -1;
      const list = videoInteractiveHistorialRef.current.entries as Record<string, unknown>[];
      const existingIdx = list.findIndex((e) => e.questionIndex === qIdx);
      if (existingIdx >= 0) list[existingIdx] = current;
      else list.push(current);
      const historial = [...list];
      const evaluated = evaluateActivityResponse(actBlock.actividad.tipo, actBlock.actividad, response);
      const qCorrect =
        evaluated.details.find((d) => d.index === qIdx)?.correct ?? evaluated.correct;
      showPill(qCorrect === true ? 'correct' : qCorrect === false ? 'incorrect' : 'sent');
    ...
    if (isActivityDraftResponse(response)) {
      respondedSlideIds.current.add(activeSlide.id);
      saveProgress({
        studentId,
        slideId: activeSlide.id,
        response: unwrapActivityDraftResponse(response),
        attemptNumber,
        activityType,
        draft: true,
      });
    ...
    const evaluated = evaluateActivityResponse(actBlock.actividad.tipo, actBlock.actividad, response);
    showPill(
      evaluated.correct === true ? 'correct' : evaluated.correct === false ? 'incorrect' : 'sent',
    );
