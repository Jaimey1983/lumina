...
import { FONT_CATALOG, FONT_DEFAULT } from '@/lib/font-catalog';
import {
  Select,
  SelectContent,
  SelectItem,
  ...
  return (
    <div className="space-y-3 border-b border-border pb-3">
      ...
      <div className="space-y-1.5">
        <span className="px-1 text-[10px] font-medium text-muted-foreground">Fuente</span>
        <Select
          value={lastText?.fuente ?? FONT_DEFAULT}
          onValueChange={(fuente) => commit({ fuente })}
          disabled={disabled}
        >
          <SelectTrigger className="h-8 text-xs w-full">
            <SelectValue>
              <span style={{ fontFamily: lastText?.fuente ?? FONT_DEFAULT }}>
                {lastText?.fuente ?? FONT_DEFAULT}
              </span>
            </SelectValue>
          </SelectTrigger>
